...
export namespace PaperGraphic$ {
  ...
  export function From(path: paper.PathItem) {
    const brush = Brush$.Purify(path.fillColor as paper.Color, path);
    const stroke = Stroke$.Purify(path);
    const shadow = Shadow$.Purify(path);
  ...
  export function To(element: PaperItemObject) {
    const ctor = GetItemType(element.type);
    const renderer = new ctor!() as PaperItemRenderer;
    renderer.Render(element);
    return renderer.visual;
  }

  export function Unite(item1: PathItemObject, item2: PathItemObject) {
    const visual1 = To(item1) as paper.PathItem;
    const visual2 = To(item2) as paper.PathItem;
