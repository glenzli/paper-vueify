...
export default class extends Vue {
  ...
  public symbols = Generic.Clone(INITS).map((coordinate) => SymbolItem({ key: this.definitions[this.selected].key, coordinate, selectable: false }));
  public counter = 0;

  get buttons() {
    const average = this.size.width / 7;
  ...
  public Select(index: number) {
    this.selected = index;
    const pivot = this.buttons[this.selected].coordinate.position;
